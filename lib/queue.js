...
Queue.prototype.dequeue = function (options, callback) {
    ...
    this.mewtwo.acquire("dequeue", function (err, lock) {
        if (err) return callback();

        self.getDequeuedJobsQueries( function (err, queries) {
            if (err) return callback(err);
            ...
            self.collection.findAndModify({
                ...
                self.mewtwo.release(lock, function(err) {
                    callback(err, self.job(doc));
                });
            });
        });
    });
